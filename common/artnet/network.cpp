...
int artnet_net_recv( node n, artnet_packet p, int delay )
{
	...
	if( result == 0 )
	{
		...
		while( msghdr = WSA_CMSG_NXTHDR( &msg, msghdr ) )
		{
			...
			case IP_PKTINFO:
			{
				...
				len = sizeRecvd;
				if( !isInputFromAdapterAllowed( pktinfo->ipi_ifindex, n->filterAdapter ) )
				{
					p->length = 0;
					return ARTNET_EOK;
				}
		...
		messageHeader->cmsg_type == IP_RECVPKTINFO )
	{
		...
		if( !isInputFromAdapterAllowed( packetInfo->ipi_ifindex, n->filterAdapter ) )
		{
			p->length = 0;
			return ARTNET_EOK;
		}
...
int artnet_net_send( node n, artnet_packet p )
{
	...
	/**
	 * This is the library's original way of sending data. But we need to make sure that data gets sent
	 * using a specific adapter so we need to use sendmsg and provide the packet info containing which
	 * interface and ip should be used to send the data.
	 *
	...
	int result = WSASendMsg( n->sd, &msg, 0, &sizeToSend, NULL, NULL );
	if( result == 0 )
	{
		ret = sizeToSend;
	}
